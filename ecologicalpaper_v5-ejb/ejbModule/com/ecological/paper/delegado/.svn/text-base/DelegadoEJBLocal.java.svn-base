 ...
 */
@Local
public interface DelegadoEJBLocal {
	...
	public void createSubVersionUsuario(SubVersionUsuario subVersionUsuario) throws ExceptionSubVersion ;
	...
	java.util.List<Seguridad_Role_Lineal> buscarTodosLosRolesTree(Tree tree,
			Role role);
	...
	void arreglarCamposNullInTablas();
	...
	List<FlowControlByUsuarioBean> find_allFlowControlByRoleBean(
			Role role);
	...
	ExtensionFileHijos find_ExtensionFileByExtension(ExtensionFileHijos extensionFile);
	...
	File filetFlowAttachment(Flow_ParticipantesAttachment doc_detalle) throws SQLException;
	...
	void sustituirFlowParticipantesOldByOtroFlows(Flow_Participantes flow_Participante, Usuario nuevo,String comentario);
	...
	Flow_Participantes devolucionFirmaFlow(Flow_Participantes flow_Participante);
	...
	List findAllSvnRutasRelativasFiles(Flow_ParticipantesAttachment flow_ParticipantesAttachment);
	SvnRutasRelativasFiles findSvnRutasRelativasFiles(Flow_ParticipantesAttachment 
			flow_ParticipantesAttachment,String archivo );
SvnRutasRelativasFiles findSvnRutasRelativasFiles(Doc_maestro doc_maestro,String archivo );
	...
	void findFlowParticipantesFirmadosAndFlowIspendiente(Flow flow) ;
	 ...
	 void devolvemos(Flow_Participantes flow_Participante,Flow_Participantes devolver);
	...
	FlowParalelo findPlantillaDoc_DetallePlantillaInFlowParalelo(FlowParalelo objeto);
	Tree findEnNivelArribaTreeDeAcuerdoSuTipo(Tree tree,long tiponodo);
	 ...
	 Solicitudimpresion findSolicitudimpresionByFlow(Solicitudimpresion solicitudimpresion);
    Solicitudimpresion findSolicitudimpresionByDocDetalle(Solicitudimpresion solicitudimpresion);
    List<SolicitudImpPart> solicitudImpPartsCancelar(
   ...
   void putPublicoDocumento(Doc_detalle doc_detalle,List<Role> roleSeleccionados,
			Usuario publicador) throws EcologicaExcepciones ;
...
throws EcologicaExcepciones;
	 ...
	 List<SolicitudImpPart> findAllsolicitudImpPartsEmpresa(
		...
		void llenarDatosRole(Role role,List<Operaciones> visibleItems);
