 ...
 * @author simon
 */
public class ClienteDocumentoGenerarAttachment extends HttpServlet {
    
    private ServicioDelegado delegado =new ServicioDelegado();
    //private Doc_maestro maestro = null;
    private Flow_ParticipantesAttachment detalle = null;
    ...
    public ClienteDocumentoGenerarAttachment() {
    ...
    throws ServletException, IOException {
        ...
        Flow_ParticipantesAttachment detalle = null;
        if (session != null) {
            
            detalle = (Flow_ParticipantesAttachment) session.getAttribute("attachment");
        ...
        String extension = "";
        if (detalle != null && detalle.getNameFile() != null && detalle.getNameFile().lastIndexOf(".") != -1) {
            extension = detalle.getNameFile().substring(detalle.getNameFile().lastIndexOf(".") + 1, detalle.getNameFile().length());
        }
        ...
        StringTokenizer extEspeciales = new StringTokenizer(confmessages.getString("extEspeciales"), ",");
...
//        }
      ...
      //      maestro = detalle.getDoc_maestro();
            ...
            String ruta = carpeta_compartida;//confmessages.getString("carpeta_compartida");// + "\\" + "doc_detalle" + "\\" + detalle.getCodigo();
            ...
            try {
                //borramos cualquier fichero del mismo tipo borrado anteriormente
                //File ficheroBorra = new File(path + "\\" + detalle.getNameFile());
                File ficheroBorra = new File(path + "/" + detalle.getNameFile());
                ficheroBorra.delete();
                //getExtensionFile(detalle.getNameFile());
                if (("pdf".equalsIgnoreCase(extension)) || ("application/pdf".equalsIgnoreCase(detalle.getContentType()))) {
                    
                    //SE GRABA EN DISCO
                    if (detalle.getAttachmentDoc()==null || detalle.getAttachmentDoc().getBinaryStream()==null){
                        saveFileInDisk(detalle, null, path, detalle.getNameFile());
                    }else{
                        saveFileInDisk(detalle, detalle.getAttachmentDoc().getBinaryStream(), path, detalle.getNameFile());
                    }
                    ...
                    if ("onlyView".equalsIgnoreCase(onlyView)) {
                        ...
                        //hacemos la conversion para tratar en lo mas minimo de que no lo modifiquen
                        //siempore van hacer .doc, para que sean convertidos
                        HashMap queTipoContextType = new HashMap();
                        if (contextSessionRequest.extensionAcepotadaToConverter(extension, queTipoContextType)) {
                            ...
                            if ("true".equals(confmessages.getString("pdftrue"))) {
                                docPDF = convertirToPDF.convertirDocumento(path, detalle.getNameFile(),detalle.getFlowParticipantes().getFlow().getDoc_detalle(),true);
                                detalle.setContentType("application/pdf");
                        ...
                        //SE GRABA EN DISCO
                        //no es lectura y se va a modificar con su respectivo formato
                        if (detalle.getAttachmentDoc()==null || detalle.getAttachmentDoc().getBinaryStream()==null){
                            saveFileInDisk(detalle, null, path, detalle.getNameFile());
                        }else{
                            saveFileInDisk(detalle, detalle.getAttachmentDoc().getBinaryStream(), path, detalle.getNameFile());
            ...
            response.setContentType(detalle.getContentType());
            InputStream in = null;
            OutputStream out = null;
            
            
            StringBuffer action = new StringBuffer();
            action.append("http://").append(request.getServerName()).append(":").append(request.getServerPort());
            action.append(request.getContextPath());
            
            String fileURL = new String();
            
//          fileURL =  action.toString() + "/" + confmessages.getString("carpeta_compartida") + "/" + detalle.getNameFile();
            //System.out.println("System.getProperty(java.io.tmpdir="+System.getProperty("java.io.tmpdir"));
            fileURL = action.toString() + "/" + tmp + "/" + detalle.getNameFile();
            ...
            try {
                ...
                if (onlyRead) {
                    response.setHeader("Content-disposition", "inline; filename=\"" + detalle.getNameFile() + "\"");
                } else {
                    response.setHeader("Content-disposition", "attachment; filename=\"" + detalle.getNameFile() + "\"");
                }
                ...
                if (!sendURL || (!onlyRead)) {
                    ...
                    /*   System.out.println("path + \"\\\" + detalle.getNameFile()="+path + "\\" + detalle.getNameFile());
                    System.out.println("fichero.getPath()="+fichero.getPath());
                    ...
                    response.setHeader("Content-Type", detalle.getContentType());
    ...
    public String getExtensionFile(String nameFile) {
        ...
        if (pos != -1) {
            ...
            extension = nameFile.substring(nameFile.lastIndexOf(".") + 1, nameFile.length());
        }
        return extension;
    }
    ...
    public void saveFileInDisk(Flow_ParticipantesAttachment doc_detalle, InputStream imagenBuffer, String path, String nameFile) {
    	   File fichero =null;
        try {
            ...
            if (swConfiguracionHayData){
                if (swPostgresVieneDeConfiguracion){
                    ByteArrayInputStream stream = new ByteArrayInputStream(doc_detalle.getAttachmentDocPostgres());
