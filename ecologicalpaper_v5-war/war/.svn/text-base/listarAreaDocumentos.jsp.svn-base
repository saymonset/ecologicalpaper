	...
	<h:form>
		...
		<h:panelGrid columns="3" binding="#{backing_login.panelGrid1}"
			...
			<h:commandButton styleClass="boton"
				rendered="#{areaDocumentosBean.swAdd}" value="#{txt.btn_crear}"
		...
		<a4j:region renderRegionOnly="true" id="stat3">
			...
			<h:panelGroup id="body">
				...
				<t:dataTable id="data" styleClass="scrollerTable"
					...
					var="car" value="#{areaDocumentosBean.areaDocumentosLsta}"
					...
					<h:column>
						...
						<h:panelGroup rendered="#{car.delete}">
							<h:commandLink id="del" action="#{areaDocumentosBean.delete}"
								actionListener="#{areaDocumentosBean.selectObjeto}"
								onclick="if (!confirm('Are you sure you want to delete this event?')) return false">
                                        >
                                        <h:graphicImage
									styleClass="node" title="#{txt.btn_erase}"
									url="#{conftxt.img_erase}" />
