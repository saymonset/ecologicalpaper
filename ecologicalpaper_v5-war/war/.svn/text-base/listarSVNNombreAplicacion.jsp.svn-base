	...
	<h:form>
		<h:panelGrid columns="3" binding="#{backing_login.panelGrid1}"
			...
			<h:commandButton styleClass="boton" value="#{txt.btn_menu}"
				action="#{svnNombreAplicacionCliente.cancelarLista}" />
			<h:commandButton styleClass="boton" rendered="#{svnNombreAplicacionCliente.swAdd}"
				value="#{txt.btn_crear}"
				action="#{svnNombreAplicacionCliente.crear_nuevo}" />
		</h:panelGrid>

		<h1><h:outputText styleClass="form"
			value="#{txt.svnnombreaplicacion}" /></h1>
		<a4j:region renderRegionOnly="true" id="stat3">
			...
			<h:panelGroup id="body">
				...
				<t:dataTable id="data" styleClass="scrollerTable"
					...
					var="car" value="#{svnNombreAplicacionCliente.lista}"
					preserveDataModel="false" rows="#{Utilidades.verNumeroDeRegistros}">
					<h:column>
						<f:facet name="header">
							<h:graphicImage styleClass="grees" title="" url="#{conftxt.img_editar}" />
						</f:facet>
						<h:panelGroup rendered="#{svnNombreAplicacionCliente.swMod}">
							<h:commandLink id="Edit"
								action="#{svnNombreAplicacionCliente.edit}"
								actionListener="#{svnNombreAplicacionCliente.selectId}">
					...
					<h:column>
						...
						<h:panelGroup rendered="#{car.delete}">
							<h:commandLink id="del"
								action="#{svnNombreAplicacionCliente.delete}"
								actionListener="#{svnNombreAplicacionCliente.selectId}"
